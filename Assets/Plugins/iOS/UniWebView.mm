...
@property (nonatomic, assign) UniWebView* instance;
...
-(void) addManagedWebViewName:(NSString *)name insets:(UIEdgeInsets)insets {
    ...
    UIImage* bg=[UIImage imageNamed:@"back.png"];
    UIButton *backBtn = [[UIButton alloc]init];
    [backBtn setBackgroundImage:bg forState:UIControlStateNormal];
    float w=25;
    float h=25;
    float w0=320;
    w=w/(w0/rect.size.width);
    h=w;
    backBtn.frame = CGRectMake(10, 10, w, h);
    [unityView addSubview:backBtn];
    
    objc_setAssociatedObject(backBtn, "webView", webView, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    [backBtn addTarget:self action:@selector(onClickBack:) forControlEvents:UIControlEventTouchUpInside];
    NSLog(@"ScreenSize:%f,%f",rect.size.width,rect.size.height);
}
		
//点击返回按钮
- (void)onClickBack:(UIButton*)sender{
    UniWebView *webView=(UniWebView*)objc_getAssociatedObject(sender,"webView");
